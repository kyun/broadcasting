    ...
    <DeviceList>
      {
        deviceInfo.audioinput?.map((item: any, index: number) => {
          return <ListItem onClick={()=>onClick('audioinput', index)} key={item.devcieId} selected={value.audioinput === index}>{item.label}</ListItem>
        })
      }
    </DeviceList>
    <hr />
    ...
    <DeviceList>
      {
        deviceInfo.videoinput?.map((item: any, index: number) => {
          return <ListItem onClick={()=>onClick('videoinput', index)} key={item.devcieId} selected={value.videoinput === index}> {item.label}</ListItem>
        })
      }
    </DeviceList>
  </Base>
